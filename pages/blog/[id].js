...
// import { useRouter } from 'next/router';
 ...
 function Post({data}) {
//amr data k next.js a useRouter ar dhorte pari
  // const [user,setUser] = useState([]);
  // const {id} = useRouter().query;
  // useEffect(()=>{
  //   fetch(`http://localhost/wp-headless/wordpress/wp-json/wp/v2/posts/${id}`)
  //   .then((res)=>res.json())
...
export async function getStaticProps(ctx){
  const {params} = ctx;
  const res = await fetch(`http://localhost/wp-headless/wordpress/wp-json/wp/v2/posts/${params.id}`);
